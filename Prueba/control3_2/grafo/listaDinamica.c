...
void linkedListInsertInPosition(struct linkedList *listInput, int elementToInsert, int position){ // ok
    ...
    if(isEmpty(listInput) == 1){
        ...
        LinkedListInsertaInicio(listInput, crearNodo(elementToInsert,1));
        
    }
    
    else {

        struct nodo *nodoToInsert = crearNodo(elementToInsert,1); // nodo que sera insertado en la lista dinamica
...
struct nodo* buscaMinimo(struct linkedList *listInput){

    int min = 999999;
    struct nodo *aux;
    struct nodo *temp = listInput->head;
    int flagFirst =1;
    if (temp->costo < min) // si el primer elemento es menor que minimo reemplaza el valor de minimo
    {
        min = temp->costo;
        aux = temp;
    }
    ...
    {
        if(temp->next->costo < min){ //si el costo del siguiente nodo es menor que el minimo reselecciona el minimo
            min = temp->next->costo;
            aux = temp;
            flagFirst = 0 ;
        }
    ...
    // en esta instancia, ya tenemos el menor nodo
    ...
    {
        return temp->next;
        temp -> next = temp->next->next;// se desengancha el nodo de la lista
    }
