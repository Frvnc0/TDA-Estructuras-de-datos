...
void linkedListDeleteInPosition(struct linkedList *listInput, int search){
    ...
    if(isEmpty(listInput) == 0){// si es que no esta vacia //!
        ...
        if (auxToBeDeleted != NULL){
            ...
            if(auxToBeDeleted == listInput->head){
                ...
                {
                    auxToBeDeleted = anterior(listInput, search); //! me falto poner un todo cuando deje de codificar ypor lo tanto no codifique las dos lineas siguientes
                    printf("%d",auxToBeDeleted->dato);
                    auxToBeDeleted->next = auxToBeDeleted->next->next;
