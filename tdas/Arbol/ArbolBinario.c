...
char *leerArchivo(char *nombreArchivo, int *quantifyOfnodes)
{
    ...
    char *arrRepresentationReturn = (char*)malloc(sizeof(char)); // se reserva el espacio para el array que sera retornado
    int stop = *quantifyOfnodes;
    int aux;

    for (int i = 0; i < stop; i++) // ! posible bug
    {
        fscanf(fp, "%c", &arrRepresentationReturn[i]);
    }

    return arrRepresentationReturn;
}

void preOrden(struct node *treeInput){
    ...
    {
        printf("%c\n", treeInput->info->dato);
        preOrden(treeInput->leftSon);
        preOrden(treeInput->rightSon);
    }

}

void inOrden(struct node *treeInput){
    ...
    {
        printf("%c\n", treeInput->info->dato);
        inOrden(treeInput->leftSon);
        inOrden(treeInput->rightSon);
    }
    
}

void postOrden(struct node *treeInput){
    ...
    {
        printf("%c\n", treeInput->info->dato);
        postOrden(treeInput->leftSon);
        postOrden(treeInput->rightSon);
    }
...
//todo: se realizan verificaciones de los niveles de los nodos correspondientes
    ...
    printf("La profundidad del nodo %c corresponde a: %d", deepOfNode(tree, 'B'));
